...
```
cd buzkashi
mkvirtualenv --no-site-packages buzkashi
...
**IMPORTANT NOTE: This project relies on an outdated method to access content in Google Spreadsheets. For now, the connection has been disabled (see `update_copy()` in `fabfile/__init__.py`), and the project instead pulls from a spreadsheet stored in `www/assets/copy.xlsx`. Run `fab assets.sync` to download this file (and other media files) from the [assets rig](#save-media-assets).**
